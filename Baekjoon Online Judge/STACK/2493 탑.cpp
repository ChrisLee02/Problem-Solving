...
//내림차순이 유지되도록 스택을 쌓다가 깨지는 순간에 내림차순이 다시 유지될 때까지 pop한다.
//스택의 응용 - 오름차순/내림차순이 깨지는 순간 앞선 인풋을 덮어써버릴 수 있는 경우일 떄
...
int main() {
    ...
    stack.push(pair<int, int>(0, 0)); // 번호, 높이

    int *tower = new int[N + 1]; // tower[i] => i번 탑의 신호를 수신한 탑의 번호

    for (int i = 1; i < N + 1; ++i) {
        cin >> height;
        //cout << stack.top().first << " " << stack.top().second << endl;
        if (stack.top().first == 0 || stack.top().second >= height) {
           // cout << "if" << endl;
            stack.push(pair<int, int>(i, height));
        } else {
            //cout << "else" << endl;
            while(stack.top().first != 0 && stack.top().second < height) {
                int tmp = stack.top().first;
                stack.pop();
                //cout << "else " << 짬통 <<" "<< stack.top().first << endl;
                tower[tmp] = stack.top().first;
            }
            stack.push(pair<int, int>(i, height));
        }
    }
    while(stack.top().first != 0){
        int tmp = stack.top().first;
        stack.pop();
        tower[tmp] = stack.top().first;
    }
