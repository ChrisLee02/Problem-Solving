...
int main()
{
    ...
    for (int test = 0; test < T; ++test) {
        unordered_map<string, int> costumes_to_count{};
        unordered_set<string> costumes{};
        int n;
        cin >> n;
        for (int i = 0; i < n; ++i) {
            string tmp1, tmp2; cin >> tmp1 >> tmp2;
            if(costumes.find(tmp1) == costumes.end()) {
                costumes_to_count[tmp2]++;
            }
        }
        int ans = 1;
        for(auto const &kv: costumes_to_count) {
            ans = ans * (kv.second+1);
        }
        cout<<ans-1 << endl;
