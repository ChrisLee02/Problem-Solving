...
//벽이 1이다.
    ...
    if (Visit[N][M] == 1) {
        dist_vec.push_back(Dist[N][M]);
    }

    for (Pair wall: walls) {
        Board[prev.X][prev.Y] = 1; // 이전에 0으로 만들었던 벽은 복원
    ...
    if (dist_vec.empty()) {
        ...
        cout << *min_element(dist_vec.begin(), dist_vec.end());
    }
