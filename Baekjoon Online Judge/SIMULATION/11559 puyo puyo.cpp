/*
뿌요뿌요의 룰은 다음과 같다.
필드에 여러 가지 색깔의 뿌요를 놓는다. 뿌요는 중력의 영향을 받아 아래에 바닥이나 다른 뿌요가 나올 때까지 아래로 떨어진다.
뿌요를 놓고 난 후, 같은 색 뿌요가 4개 이상 상하좌우로 연결되어 있으면 연결된 같은 색 뿌요들이 한꺼번에 없어진다. 이때 1연쇄가 시작된다.
뿌요들이 없어지고 나서 위에 다른 뿌요들이 있다면, 역시 중력의 영향을 받아 차례대로 아래로 떨어지게 된다.
아래로 떨어지고 나서 다시 같은 색의 뿌요들이 4개 이상 모이게 되면 또 터지게 되는데, 터진 후 뿌요들이 내려오고 다시 터짐을 반복할 때마다 1연쇄씩 늘어난다.
터질 수 있는 뿌요가 여러 그룹이 있다면 동시에 터져야 하고 여러 그룹이 터지더라도 한번의 연쇄가 추가된다.
남규는 최근 뿌요뿌요 게임에 푹 빠졌다. 이 게임은 1:1로 붙는 대전게임이라 잘 쌓는 것도 중요하지만, 상대방이 터뜨린다면 연쇄가 몇 번이 될지 바로 파악할 수 있는 능력도
 필요하다. 하지만 아직 실력이 부족하여 남규는 자기 필드에만 신경 쓰기 바쁘다. 상대방의 필드가 주어졌을 때, 연쇄가 몇 번 연속으로 일어날지 계산하여 남규를 도와주자!

입력
총 12개의 줄에 필드의 정보가 주어지며, 각 줄에는 6개의 문자가 있다.
이때 .은 빈공간이고 .이 아닌것은 각각의 색깔의 뿌요를 나타낸다.
R은 빨강, G는 초록, B는 파랑, P는 보라, Y는 노랑이다.
입력으로 주어지는 필드는 뿌요들이 전부 아래로 떨어진 뒤의 상태이다. 즉, 뿌요 아래에 빈 칸이 있는 경우는 없다.

출력
현재 주어진 상황에서 몇연쇄가 되는지 출력한다. 하나도 터지지 않는다면 0을 출력한다.
 */

#include <bits/stdc++.h>

#define Pair pair<int,int>
#define X first
#define Y second

using namespace std;

int Board[12][6];
int Board_copied[12][6];
int dx[4] = {0, 1, 0, -1};
int dy[4] = {1, 0, -1, 0};

int seq = 0;

bool isValidCoordinate(int a, int b) {
    return a >= 0 && a < 12 && b >= 0 && b < 6;
}

void slide(int a, int b) {
    for (int i = a; i >= 1; --i) {
        Board[i][b] = Board[i - 1][b];
    }
    Board[0][b] = '.';

}

void update() { // 터트린 후 이동.
    for (int i = 0; i < 12; ++i) {
        for (int j = 0; j < 6; ++j) {
            if (Board[i][j] == '#') {
                slide(i, j);
            }
        }
    }
}

bool BFS(Pair start, int color) {
    int visit[12][6];
    for (int i = 0; i < 12; ++i) {
        for (int j = 0; j < 6; ++j) {
            visit[i][j] = 0;
        }
    }
    vector<Pair> List{};
    queue<Pair> Q{};
    Q.push(start);
    visit[start.X][start.Y] = 1;
    List.emplace_back(start.X, start.Y);

    while (!Q.empty()) {
        Pair cur = Q.front();
        Q.pop();
        for (int i = 0; i < 4; ++i) {
            int nx = cur.X + dx[i];
            int ny = cur.Y + dy[i];
            if (isValidCoordinate(nx, ny) && !visit[nx][ny] && Board[nx][ny] == color) {
                Q.emplace(nx, ny);
                visit[nx][ny] = 1;
                List.emplace_back(nx, ny);
            }
        }
    }

    if (List.size() >= 4) {
        for (Pair tmp: List) {
            Board[tmp.X][tmp.Y] = '#';
        }
        return true;
    } else return false;


}

bool pang() { // 터트림
    bool result = false;
    for (int i = 0; i < 12; ++i) {
        for (int j = 0; j < 6; ++j) {
            if (Board[i][j] == '.' || Board[i][j] == '#') continue;

            Pair start = Pair(i, j);
            int color = Board[i][j];

            result = BFS(start, color) || result; // 순서가 바뀌면 안된다. short-circuit 문제.

        }
    }
    return result;
}

void fun() {
    while (pang()) {
        //cout <<" dd";
        seq++;
        /*for (int i = 0; i < 12; ++i) {
            for (int j = 0; j < 6; ++j) {
                cout << (char)Board[i][j];
            }
            cout << endl;
        }
        cout << endl<<endl;*/
        update();
    }
}

int main() {
    string tmp;
    for (int i = 0; i < 12; ++i) {
        cin >> tmp;
        for (int j = 0; j < 6; ++j) {
            Board[i][j] = tmp[j];
        }
    }

    fun();
    cout << seq;


}
