...
출력
출력은 표준출력을 사용한다. 각 테스트 데이터에 대해, 모든 연산을 처리한 후 Q에 남아 있는 값 중 최댓값과 최솟값을 출력하라. 두 값은 한 줄에 출력하되
 하나의 공백으로 구분하라. 만약 Q가 비어있다면 ‘EMPTY’를 출력하라.
...
private:
multiset<int> tree; //set은 Balanced binary tree
public:
    ...
    void max_pop() {
        ...
        tree.erase(--tree.end()); // 포인터로 해당 자리를 지우는 것도 가능하다. 이때 인자로는 iterator만 받음.
        // 값으로 접근하면 중복된 요소를 다 지워서 곤란함.
    }
...
int main() {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        if (doublePriorityQueue.empty()) {
            ...
            cout << *doublePriorityQueue.rbegin() << " " << *doublePriorityQueue.begin() << '\n';
        };
