...
입력
첫째 줄에는 지도의 세로의 크기 M과 가로의 크기 N이 빈칸을 사이에 두고 주어진다. 이어 다음 M개 줄에 걸쳐 한 줄에 N개씩 위에서부터 차례로
 각 지점의 높이가 빈 칸을 사이에 두고 주어진다. M과 N은 각각 500이하의 자연수이고, 각 지점의 높이는 10000이하의 자연수이다.
...
int dx[4] = {1, 0, -1, 0};
int dy[4] = {0, 1, 0, -1};
int dp[501][501]; //dp[i][j] = 1,1에서 i,j까지 경로의 개수
int dp_2[501][501]; // dp[i][j] = i,j에서 n,m까지 경로의 개수
int Array[501][501];
int n, m;
...
int fun(int i, int j) {
    ...
    for (int k = 0; k < 4; ++k) {
        int nx = i + dx[k];
        int ny = j + dy[k];
        if(isValid(nx, ny) && Array[i][j] < Array[nx][ny] ) {
            ret += fun(nx,ny);
        }
    }
    return ret;
