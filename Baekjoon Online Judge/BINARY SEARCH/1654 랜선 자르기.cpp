...
첫째 줄에는 오영식이 이미 가지고 있는 랜선의 개수 K, 그리고 필요한 랜선의 개수 N이 입력된다. K는 1이상 10,000이하의 정수이고,
 N은 1이상 1,000,000이하의 정수이다. 그리고 항상 K ≦ N 이다. 그 후 K줄에 걸쳐 이미 가지고 있는 각 랜선의 길이가 센티미터 단위의 정수로 입력된다.
...
int param_search() {
    ...
    while(left<right) {
        int mid = ( (long long)left + (long long)right + 1) / 2; //여기서 left, right이 int형이라 오버플로우가 발생함.
        // 계산할 때만 long long으로 캐스팅해주면 됨.
        if(f(mid) >= N) {
            left = mid;
        } else {
            right = mid - 1;
